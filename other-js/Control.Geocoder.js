...
this.L.Control.Geocoder = (function (L) {
  ...
  function getJSON(url, params, callback, errCallback) { // last one (errCallBack) by cic
    ...
    // added by cic to call errCallback in case of errors
    xmlHttp.onerror = function () {
	console.log("** Geocoder: error on getJSON "  + xmlHttp.status);
	errCallback();
    };
  ...
  var Nominatim = L.Class.extend({
    ...
    geocode: function(query, cb, context) {
      // by cic remove via viale piazza corso
      query=query.replace(/^via|^viale|^piazza|^corso/gi,'');
      getJSON(
  ...
  var Photon = L.Class.extend({
	...
	L.bind(function(data) { // added by cic
	  console.log("** Photon error, trying with next if available");
          if (this.options.next) { this.options.next.geocode(query, cb, context); }
        }, this)
      );
    },
  ...
  var Geocoder = L.Control.extend({
    options: {
      ...
      collapsed: true,
      expand: 'touch', // options: touch, click, anythingelse // by cic changed default to click this seems to interfere with #6885
      position: 'topright',
      placeholder: 'Search...',
    ...
    onAdd: function(map) {
      ...
      // by cic
      L.DomEvent.addListener(container, "click",  function(e) {this._toggle();L.DomEvent.stopPropagation(e); }, this );       
      /* by cic all commented and added line above
      ...
      ***/
      ...
      // by cic next two line enable paste or click on textfield from mobile devices w/o hitting below map...
      L.DomEvent.addListener(input, "contextmenu click",  function(e) {L.DomEvent.stopPropagation(e); } ); 
      L.DomEvent.addListener(container, "contextmenu click",  function(e) {L.DomEvent.stopPropagation(e); } ); 
    ...
    _createAlt: function(result, index) {
	  ...
	  /* by cic line below added and commented next */
